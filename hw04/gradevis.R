...
ui <- fluidPage(
   ...
   sidebarLayout(
     sidebarPanel(
      ...
      # Show plots of the generated distributions depending on the tab
   mainPanel(
...
server <- function(input, output) {
  ...
  proportions <- c()
  for (i in 1:10) {
    frequencies <- append(frequencies, grade_table[[i]])
    proportions <- append(proportions, round(grade_table[[i]] / total_grades, digits = 2))
  }
  
  grade_dists <- data.frame(grades, frequencies, proportions)
  ...
  vis_scatter <- reactive({
    ...
    scatters <- rawmatrix %>% ggvis(x = var2, y = var3, opacity := input$inputOpacity) %>% layer_points()
    if (input$type5 == "lm") {
      scatters = scatters %>% layer_model_predictions(model = "lm", stroke := "purple")
    }
    else if (input$type5 == "smooth") {
      scatters = scatters %>%  layer_model_predictions(model = "loess", stroke := "purple")
  }
  scatters
  })
